...
public:
	...
	ofxPanel gui;
    ofParameter<bool> toggleBlobDetection = { "Enable Blob Detection", false };
    ofxCv::ContourFinder finder, contourFinder;
    ofxCv::RunningBackground background;
    ofImage thresholded;

    ofParameter<bool> colorize = { "Colorize", false };
    ofParameter<int> blurAmount = { "Blur Amount", 1, 1, 100 };
    ofParameter<float> cloud1rotX = { "Cloud 1 X Angle", 0.0, 0.0, 360.0 };
    ofParameter<float> cloud1rotY = { "Cloud 1 Y Angle", 180.0, 0.0, 360.0 };
    ofParameter<float> cloud1rotZ = { "Cloud 1 Z Angle", 270.0, 0.0, 360.0 };
    ...
    ofParameterGroup openCvGuiGroup;
    ofxPanel cvGui;
    ofParameter<float> openCvMinAreaRadius = { "Min Area Radius", 1, 1, 100 };
    ofParameter<float> openCvMaxAreaRadius = { "Max Area Radius", 100, 1, 100 };
    ofParameter<float> openCvThreshold = { "Threshold Amount", 45, 1, 255 };
    ofParameter<float> openCvPersistence = { "Persistence", 15, 1, 100 };
    ofParameter<float> openCvMaxDistance = { "Max Distance", 32, 1, 128 };
    ofParameter<float> openCvLearningTime = { "Min Area Radius", 25, 0, 60 };
    ofParameter<bool> useSubtraction = { "Subtract ", true };

    ofParameter<bool> showDepthTexture = { "Show Depth Texture", false };
    ofParameter<bool> showBlobTexture = { "Show Blob Texture", false };
